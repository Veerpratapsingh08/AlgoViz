    ...
    <div className="flex flex-col h-full w-full relative">
        ...
        <div className="absolute top-24 md:top-28 left-1/2 transform -translate-x-1/2 w-[95%] md:w-auto max-w-5xl z-30">
            <div className="sketch-box bg-stone-800 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 p-4 md:px-8 md:py-4 -rotate-1">
                ...
                <div className="flex items-center gap-2 w-full md:w-auto">
                    <button onClick={() => switchType('bst')} className={`btn ${type === 'bst' ? 'sketch-box bg-stone-700 text-stone-100' : 'bg-transparent text-stone-400 hover:text-stone-100 border-transparent'}`}>BST</button>
                    <button onClick={() => switchType('stack')} className={`btn ${type === 'stack' ? 'sketch-box bg-stone-700 text-stone-100' : 'bg-transparent text-stone-400 hover:text-stone-100 border-transparent'}`}>Stack</button>
                    <button onClick={() => switchType('queue')} className={`btn ${type === 'queue' ? 'sketch-box bg-stone-700 text-stone-100' : 'bg-transparent text-stone-400 hover:text-stone-100 border-transparent'}`}>Queue</button>
                </div>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <input 
                      className="flex-1 sketch-box bg-stone-700 px-4 py-2 text-base text-stone-100 outline-none w-full md:w-48 placeholder:text-stone-400"
                ...
                <div className="flex items-center gap-2 justify-end mt-2 md:mt-0 flex-wrap md:flex-nowrap">
                    <button onClick={insert} className="btn btn-primary flex-1 md:flex-none justify-center px-6">
                        <span className="material-symbols-outlined text-lg">add</span>
                        <span className="tracking-wide text-xl">Insert</span>
                    </button>
                    <button onClick={remove} className="btn btn-surface flex-1 md:flex-none justify-center px-4 sketch-box bg-stone-700 hover:bg-stone-600">
                        <span className="material-symbols-outlined text-lg">remove</span>
                        <span className="tracking-wide text-xl">Remove</span>
                    </button>
                    <button onClick={search} className="btn btn-surface flex-1 md:flex-none justify-center px-4 sketch-box bg-stone-700 hover:bg-stone-600">
                        <span className="material-symbols-outlined text-lg">search</span>
                    </button>
                    <button onClick={reset} className="btn btn-surface flex-1 md:flex-none justify-center px-4 sketch-box bg-stone-700 hover:bg-stone-600" title="Clear All">
        ...
        <div className="fixed bottom-4 left-4 right-4 md:absolute md:left-8 md:top-48 md:bottom-auto md:right-auto md:w-72 z-20 flex flex-col gap-2 md:gap-4 max-h-[40vh] md:max-h-[calc(100vh-140px)] overflow-y-auto custom-scrollbar pr-0 md:pr-2 pb-0 md:pb-4 rotate-1">
            ...
            <CollapsiblePanel title="Activity Log" icon="history" initialOpen={true}>
                 <div className="sketch-box bg-stone-700 p-3 h-24 md:h-40 overflow-y-auto font-mono text-base custom-scrollbar">
                    {logs.map((msg, i) => (
                        <div key={i} className="text-stone-300 mb-1 border-b border-stone-600 pb-1 last:border-0"><span className="text-stone-400 font-bold">{">"}</span> {msg}</div>
                    ))}
                    {logs.length === 0 && <div className="text-stone-500 italic">Waiting for operations...</div>}
                </div>
            </CollapsiblePanel>
                ...
                <CollapsiblePanel title="Structure Details" icon="info" initialOpen={false}>
                        ...
                        <div className="space-y-3 border-t border-stone-600 pt-3">
                            <div className="flex justify-between items-center">
                                <span className="text-stone-400 font-medium">Access</span>
                            ...
                            <div className="flex justify-between items-center">
                                <span className="text-stone-400 font-medium">Search</span>
                                <span className="font-mono text-stone-100 bg-stone-700 px-2 py-0.5 rounded text-base border border-stone-600">
                                    {type === 'bst' ? 'O(log n)' : 'O(n)'}
                                </span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-stone-400 font-medium">Insertion</span>
                                <span className="font-mono text-stone-100 bg-stone-700 px-2 py-0.5 rounded text-base border border-stone-600">
                                    {type === 'bst' ? 'O(log n)' : 'O(1)'}
                                </span>
                            </div>
       ...
       <div className="flex-1 relative overflow-hidden flex items-center justify-center p-4 md:p-8" ref={canvas}>
           ...
           {type === 'stack' && (
               <div className="flex flex-col-reverse items-center justify-end h-3/4 w-40 sketch-box bg-stone-800 p-4 gap-3 overflow-hidden rounded-b-3xl">
                   {items.map((item) => (
                       <div key={item.id} className="w-full h-14 sketch-box bg-stone-700 flex items-center justify-center font-bold text-2xl text-stone-100">
                           {item.val}
                       </div>
                   ))}
           ...
           {/* Queue Render */}
           {type === 'queue' && (
                <div className="flex items-center justify-start max-w-4xl h-36 sketch-box bg-stone-800 p-4 gap-3 overflow-x-auto relative custom-scrollbar">
                   <div className="text-xs text-stone-400 font-bold uppercase tracking-widest absolute top-2 left-6">Front</div>
                   <div className="text-xs text-stone-400 font-bold uppercase tracking-widest absolute top-2 right-6">Back</div>
                   
                   <div className="flex items-center gap-3 pt-6 px-4">
                       {items.map((item) => (
                           <div key={item.id} className="min-w-[64px] h-14 px-4 sketch-box bg-stone-700 flex items-center justify-center font-bold text-2xl text-stone-100">
                               {item.val}
                           </div>
                       ))}
...
function RecursiveNodes({ node }: { node: BSTNode | null }) {
    ...
    return (
        <>
            <div 
                className="absolute w-12 h-12 rounded-full sketch-box bg-stone-700 border-2 border-stone-500 flex items-center justify-center font-bold text-2xl text-stone-100 transform -translate-x-1/2 -translate-y-1/2 transition-all duration-500 z-10"
